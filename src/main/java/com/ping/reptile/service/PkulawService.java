...
public class PkulawService {
    ...
    public void list(Integer pageNum, Integer pageSize) {
        ...
        LocalDate end = date.minusDays(days.get() - 1);
        LocalDate plusDays = end.plusDays(10);
        String startDate = start.getYear() + "." + start.getMonthValue() + "." + start.getDayOfMonth();
        String endDate = end.getYear() + "." + end.getMonthValue() + "." + end.getDayOfMonth();
        ...
        Node punishmentDate = Node.builder().type("daterange").order(5).showText("处罚日期").fieldName("PunishmentDate").combineAs(2).fieldItems(Lists.newArrayList(Item.builder()
                ...
                .start(startDate)
                .end(endDate)
                .values(Lists.newArrayList(startDate, plusDays.getYear() + "." + plusDays.getMonthValue() + "." + plusDays.getDayOfMonth()))
                .build())).build();
